...
export function NavMenuCommon({ pathname }: { pathname: string }) {
    return <nav className={classNames(
        ...
        <NavbarItem href="/">
            <img src="/images/logo.jpg" className="block rounded-full sm:w-full h-full sm:h-auto transition-opacity group-hover:opacity-80 group-active:opacity-60" alt="Anthony Li (anli5005) - Home" />
            <span className="sr-only">Anthony Li (anli5005) - Home</span>
            <NavbarTooltip responsiveMode="hide" activeClassName="bg-gradient-to-br from-sage-500 via-ocean-500 to-grape-600 text-white" active={pathname === "/"}>Home</NavbarTooltip>
        </NavbarItem>
        <NavbarIcon
            sizing="sm:mt-1"
            href="/blog"
